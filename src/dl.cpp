...
char isthere (double star_id) {
    ...
    sect_x *= 100000;
    sect_y = (dzat_y - visible_sectors_y * 50000) / 100000;
    sect_y *= 100000;
    sect_z = (dzat_z - visible_sectors_z * 50000) / 100000;
    sect_z *= 100000;
    asm {   mov ax, visible_sectors_x
            mov sx, ax }
    e_while:
    asm {   mov ax, visible_sectors_y
            mov sy, ax }
    m_while:
    asm {   mov ax, visible_sectors_z
            mov sz, ax }
    i_while:
    ...
    if (sts > 100) {
        sprintf (warnbuffer, "SLICE %d OF %d", visible_sectors_x - sx,
                 visible_sectors_x);
        warn (warnbuffer, 0);
...
void prepare_nearstar () {
    ...
    nearstar_identity = nearstar_x / 100000 * nearstar_y / 100000 * nearstar_z /
                        100000;
    srand ((long)nearstar_x % 10000 * (long)nearstar_y % 10000 *
           (long)nearstar_z % 10000);
...
void listplanets () {
    ...
    if (query == 1) {
        ...
        while (_read (fh, &object_id, 8) && _read (fh, &object_label, 24) == 24) {
            ...
            if (memcmp (&object_id, "Removed:", 8) && fp_object_check == fp_star_check
                    && object_id > star_id + 1 - idscale
                    && object_id <= star_id + MAX_OBJECTS_PER_STAR + idscale) {
                planet_nr  = object_label[23] - '0';
                planet_nr += 10 * (object_label[22] - '0');
        ...
        while (pcount) {
            ...
            while (mcount) {
                ...
                if (pcount) {
                    if (mcount) {
                        sprintf (textbuffer, "] $%02d&%s", nearstar_p_moonid[planet_nr - 1] + 1,
                                 object_label);
                        msg (textbuffer);
                        ncount = notesabout (object_id);
