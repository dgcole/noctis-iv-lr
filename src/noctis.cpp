        ...
        if (rx > 10 && ry > 10 && rx < 310 && ry < 190) {
            ...
            for (int16_t i = 0; i < 4; i++) {
                ...
                for (int16_t j = 7; j > 3; j--) {
                    adapted[offsetmaybe + mod1 * 320 * j + mod2 * j] = 126;
                }
            }
        }
    }
...
reset_cli:
                ...
                if (c == 36) {
                    goto non_valido;    // dollaro
                }

                if (c == 38) {
                    goto non_valido;    // e commerciale
                }

                if (c == 60) {
                    goto non_valido;    // minore
                }

                if (c == 62) {
                    goto non_valido;    // maggiore
                }

                if (c == 95) {
                    goto valido;    // sottolinea
                }
...
#define surface_crosshair_y_spacing +10
    ...
    stick3d (osscreen_x[0], osscreen_y[0],
             ...
             osscreen_z[0] - 27 * 15 + surface_crosshair_x_spacing +
             surface_crosshair_x_shift);
    stick3d (osscreen_x[0], osscreen_y[0] + 17 * 15 - surface_crosshair_y_spacing +
             surface_crosshair_y_shift, osscreen_z[0],
             osscreen_x[0], osscreen_y[0] + 17 * 15 - surface_crosshair_y_spacing +
             surface_crosshair_y_shift, osscreen_z[2]);
    stick3d (osscreen_x[0], osscreen_y[0] + 17 * 15 + surface_crosshair_y_spacing +
             surface_crosshair_y_shift, osscreen_z[0],
             osscreen_x[0], osscreen_y[0] + 17 * 15 + surface_crosshair_y_spacing +
             surface_crosshair_y_shift, osscreen_z[2]);
        ...
        if (l_dsd > 6 * nearstar_ray) {
            if (nearstar_class != 5 && nearstar_class != 6 &&
                ...
                if (nearstar_class != 11 || gl_start < 90) {
                    if (l_dsd > 5 * nearstar_ray &&
                        l_dsd < 1000 * nearstar_ray) {
        ...
        if (l_dsd < 8 * nearstar_ray) {
            ...
            if (nearstar_spin) {
                ...
                globe(gl_start, adapted, s_background, (uint8_t *)n_globes_map,
                      gl_bytes, nearstar_x, nearstar_y, nearstar_z,
                      nearstar_ray, 64, satur);
            } else {
            ...
            if (c > 0) {
                ...
                case 2: // local target data
                    if (ip_targetted != -1) {
                        ...
                        if (p1 < 2) {
                            sprintf((char *)outhudbuffer,
                                    "%ld EPOCS, %03ld:%03ld:%03ld", p1, p2, p3,
                                    p4);
                        } else {
                            if (p1 < 2047) {
                                sprintf((char *)outhudbuffer,
                                        "%ld EPOCS, %03ld:%03ld:???", p1, p2,
                                        p3);
                            } else {
                                sprintf((char *)outhudbuffer,
                                        "%ld EPOCS, %03ld:???:???", p1, p2);
                ...
                case 3: // environment data
                    ...
                    sprintf((char *)outhudbuffer, "LI+ IONS: %ld MTPD EST.",
                            ir);
                    wrouthud(14, 119, c, (char *)outhudbuffer);
#if 0
                    tmp_float = 50 + random (10) - random (10);
#endif
                    STUB tmp_float *= (1 - eclipse);
                    tmp_float *= 100 / dsd;
                    ...
                    tmp_float *= 1
                                 ...
                                 - (float)(random(100)) * 0.001;
#endif
                    STUB sprintf((char *)outhudbuffer, "RADIATION: %1.1f KR",
                                 tmp_float);
    ...
    resynctoplanet:
        ...
        if (ip_targetted != -1 && pwr > 15000) {
            ...
            if (ip_reached && nsync) {
                ...
                if (nsync == 3) { // syncrone orbit
                    ang =
                        (double)nearstar_p_rotation[ip_targetted] * (double)deg;
        ...
        stz = dzz * cos(deg * navigation_beta) -
              dxx * sin(deg * navigation_beta) - fabs(dyy) / 2;
        ...
        if (resolve == 1) {
            while (resolve <= 63) {
                tavola_colori((uint8_t *)return_palette, 0, 256, resolve,
                              resolve, resolve);
                ir = clock();
            ...
            while (tasto_premuto()) {
                ...
                goesk_a_reentry:
                    ...
                    if (targets_in_range) {
                        ...
                        if (c == 13) {
                            if (!collecting_targets &&
                                ...
                                ap_target_x = targets_table_px[topmost_target +
                                                               target_line];
                                ap_target_y = targets_table_py[topmost_target +
                                                               target_line];
                                ap_target_z = targets_table_pz[topmost_target +
                                                               target_line];
                                extract_ap_target_infos();
                                fix_remote_target();
                            }
                    ...
                    if (manual_target) {
                        ...
                        switch (mt_coord) {
                        case 0:
                            sprintf(temp_distance_buffer, "%s",
                                    manual_x_string);
                            break;
