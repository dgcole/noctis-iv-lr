...
void Rettangolo (long x, long y, long l, long a,
    ...
    if (x + l >= 0 && y + a >= 0 && x + l < 320 && y + a < 200) {
        adapted[(y + a) * 320 + x + l] = colore;
    }
}
...
unsigned char font[65][6] = {
    ...
    0, 0, 0, 0, 0, 0, // Dollaro (manca, Š un separatore verticale).
    ...
    63, 63, 63, 63, 63, 63, // Chiocciola (Š un blocco unico).
    28, 34, 34, 62, 34, 34, // A.
    60, 34, 60, 34, 34, 60, // B.
    28, 34, 32, 32, 34, 28, // C.
    60, 34, 34, 34, 34, 60, // D.
    ...
    34, 38, 38, 46, 26, 2, // Ordinale femminile (Š il simbolo dei micron).
...
void Copia (unsigned char far* dest, unsigned char far* sorg, unsigned x,
            unsigned y, int l, int a) {
    // ds:si partenza
    // es:di arrivo
    if (l < 2) {
        return;
